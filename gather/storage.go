...
func (s *natsStorage) Record(ms []Metrics) error {
	...
	if err != nil {
		return fmt.Errorf("scraper metrics serialization error: %v", err)
	}
	_, err = buf.Write(b)
	if err != nil {
		return fmt.Errorf("scraper metrics buffer write error: %v", err)
	}
	if err = s.publisher.Publish(s.subject, buf); err != nil {
		return fmt.Errorf("scraper publisher publish error: %v", err)
	}
	return nil
}
